...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model, n_head, attn_mask=None, mamol_enable=True, mamol_cfg=None):
        ...
        self.mamol = None
        if mamol_enable:
            cfg = mamol_cfg or dict(rank=16, num_dyn=4, num_stat=3, topk=2, router_dim=8)
...
class Transformer(nn.Module):
    def __init__(self, width, layers, heads, attn_mask=None,
        ...
        self.mamol_layers = set(mamol_layers or [10,11])
        # Router embedding
        router_dim = (mamol_cfg or {}).get("router_dim", 8)
        self.missing_proj = nn.Linear(3, router_dim)

        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask,mamol_enable=(mamol_enable and (i in self.mamol_layers)),) for i in range(layers)])
