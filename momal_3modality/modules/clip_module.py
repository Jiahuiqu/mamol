...
import LMMOE.modules.vision_transformer_clip as vit_moe
...
class TextEncoder(nn.Module):
    def __init__(self, clip_model):
        super().__init__()
        self.transformer = clip_model.transformer  # 将被替换为 TextTransformer
...
class CustomCLIP(nn.Module):
    def __init__(self, clip_model):
        super().__init__()
        self.image_encoder = clip_model.visual  # MoE视觉分支
    ...
    def forward(self, image, text):
        ...
        text_features = self.text_encoder(tokenized_texts)  # 传入 missing_type
...
class CLIPransformerSS(pl.LightningModule):
    ...
    def infer(
        ...
        both_feats = self.model(img, text)
