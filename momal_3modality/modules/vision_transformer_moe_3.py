    ...
    def forward(self, x, missing_embed):
        ...
        if self.num_stat > 0:
            ...
            for j in range(1, self.num_stat):
                if j - 1 < missing_embed.shape[1]:
                    act = (missing_embed[:, j - 1] > 0.5).float().view(1, B, 1)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, missing_type=None):
        ...
        mt_bits = torch.stack([
            (missing_type & 1) > 0,  # HS 
            (missing_type & 2) > 0,  # LiDAR 
            (missing_type & 4) > 0,  # SAR 
        ], dim=1).float()

        missing_embed = self.missing_proj(mt_bits)
...
def build_model(state_dict: dict):
    from .vision_transformer_clip import CLIP
