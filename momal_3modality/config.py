...
# conda activate newmissing2
# cd /media/xidian/xd132/gqhhh/Deep_Correlated_Prompting-main
# python run.py with remote
...
def config():
    ...
    # missing_ratio = {'train': 0.9, 'val': 0.9, 'test': 0.9}
    # missing_ratio = {'train': 0, 'val': 0, 'test': 0}
    # type啥，缺啥
    missing_type = {'train': 'both', 'val': 'both', 'test': 'both'} # ['text', 'image', 'both'] in VL taskss
    # missing_type = {'train': 'HS', 'val': 'HS', 'test': 'both'} # ['text', 'image', 'both'] in VL taskss
...
@ex.named_config
def remote_3modality():

    data_root = "/datasets/augsburg_3"   

    datasets = ["remote_3"]          
    exp_name = "remote_3modality"    
    ...
    missing_table_root = "./missing_tables"
    simulate_missing = False
    both_ratio = 0.5                 

    train_transform_keys = ["lidarHS_transform"]
    val_transform_keys = ["lidarHS_transform"]

    loss_names = _loss_names({"remote": 1})
