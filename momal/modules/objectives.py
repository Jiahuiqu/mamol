...
def compute_itm_wpa(pl_module, batch):
    ...
    return ret
def compute_remote(pl_module, batch):
    phase = "train" if pl_module.training else "val"
    infer = pl_module.infer(batch)
    logits = pl_module.remote_classifier(infer["cls_feats"])
    labels = batch["label"]
    # 确保是 tensor
    if not isinstance(labels, torch.Tensor):
        labels = torch.tensor(labels).to(pl_module.device).long()
    ...
    # 更新分类指标
    getattr(pl_module, f"{phase}_remote_class_metrics")(logits, labels)
    # 记录 loss scalar
    loss = getattr(pl_module, f"{phase}_remote_loss")(ret["remote_loss"])

    pl_module.log(f"remote/{phase}/loss", loss)
