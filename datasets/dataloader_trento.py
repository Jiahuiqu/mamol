...
class LIDARHS(Dataset):
    def __init__(self, patchsize, mode = 'train', classnum = 100):
        ...
        self.lidar_mat = np.squeeze(sio.loadmat('/datasets/data_trento/LiDAR_data.mat')['LiDAR_data'].astype(np.float32))
        self.lidar_mat = np.pad(self.lidar_mat, self.padding_layers, mode='symmetric')
        self.HS_mat = np.squeeze(sio.loadmat('/datasets/data_trento/HSI_data.mat')['HSI_data'].astype(np.float32))
        self.HS_mat = np.pad(self.HS_mat, ((self.padding_layers, self.padding_layers), (self.padding_layers, self.padding_layers), (0, 0)),
                        mode='symmetric')
        self.test_lable_os = '/datasets/data_trento/All_Label.mat'
