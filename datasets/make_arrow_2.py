...
def make_arrow_lidarhs(dataset, dataset_root, split="train"):
    ...
    for idx in tqdm(range(len(dataset))):
        lidar, HS, label, ik = dataset[idx]
        lidar = lidar.numpy().tolist() if hasattr(lidar, "numpy") else lidar.tolist()
        HS = HS.numpy().tolist() if hasattr(HS, "numpy") else HS.tolist()
        label = int(label) - 1 
        col, nul = int(ik[0]), int(ik[1])

        data = (lidar, HS, label, col, nul)
